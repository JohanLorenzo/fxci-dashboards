...
const userRows = relabelUntagged((await FileAttachment("data/usage.parquet").parquet()).toArray());
```
...
// on poolPattern re-run that cell too, rebuilding the control (and losing the
// selected range) on every keystroke.
{
